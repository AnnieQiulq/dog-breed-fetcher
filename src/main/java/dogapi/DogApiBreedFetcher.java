...
public class DogApiBreedFetcher implements BreedFetcher {
    ...
    public List<String> getSubBreeds(String breed) throws BreedNotFoundException{
        ...
        if (breed == null || breed.trim().isEmpty()) {
            throw new BreedNotFoundException("Breed cannot be null or empty");
        }
        String formattedBreed = breed.toLowerCase().replace(" ", "-");
        String url = "https://dog.ceo/api/breed/" + formattedBreed + "/list";
        ...
        try (Response response = client.newCall(request).execute()) {
            ...
            if (!"success".equalsIgnoreCase(json.getString("status"))) {
                throw new BreedNotFoundException("API returned error: " + json.optString("message", "Unknown error"));
            }
